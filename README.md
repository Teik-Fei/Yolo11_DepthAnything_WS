# 🌊 YOLO11-Seg + Depth Anything v3 + Octomap Fusion (ROS 2 Humble)
...
This package upgrades standard 2D detection into a full **volumetric navigation system**. It fuses **YOLO11-Seg (Instance Segmentation)** with **Depth Anything V3** to create precise 3D object clouds, builds a persistent voxel map using **Octomap**, and feeds it directly into **Nav2** for obstacle avoidance.
...
2.  **Model Preparation**
    ...
    * **YOLO11-Seg:** Export your segmentation model:
        ...
        ```
    * **Depth Anything V3:** Convert the ONNX model using `trtexec`:
        ```bash
        /usr/src/tensorrt/bin/trtexec --onnx=depth_anything_v3_vits.onnx --saveEngine=depth_anything_v3_vits_fp16.engine --fp16
        ```
...
| **`depth_anything_trt_node`** | **Depth Engine** | Runs Depth Anything V3 (TensorRT) to generate a high-quality depth map from the monocular RGB camera. |

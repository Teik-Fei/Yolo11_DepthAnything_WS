...
//=========================================
// SAUVC detection Node using TensorRT
//=========================================
...
struct YoloDet {
    ...
    float score;
    float x, y, w, h;
    cv::Mat mask; // Generated via Color Thresholding
};
...
public:
    Yolo11TrtNode() : Node("yolo11_trt_node") {
        RCLCPP_INFO(this->get_logger(), "Initializing YOLO11 HYBRID SEGMENTATION Node");

        engine_path_ = this->declare_parameter<std::string>("engine_path", "/home/mecatron/Yolo11_DepthAnything_WS/src/yolo3d_stack/models/sauvc_5.engine");
        conf_thresh_ = this->declare_parameter<float>("conf_threshold", 0.50f); 
        visualize_ = this->declare_parameter<bool>("visualize_output", true);
        ...
        // 7 Classes for SAUVC
        class_names_ = {"Blue flare", "Blue pail", "Gate", "Orange flare", "Red flare", "Red pail", "Yellow flare"};
        ...
        depth_scale_ = this->declare_parameter<float>("depth_scale_factor", 1.0f);
    }
...
private:
    ...
    // Output 1 (Unused Segmentation Proto-masks)
    ...
    // --- Hybrid Segmentation Helper ---
    cv::Mat get_color_segmentation(const cv::Mat& frame, cv::Rect box, int class_id) {
        
        cv::Mat full_mask = cv::Mat::zeros(frame.size(), CV_8UC1);
        cv::Rect safe_box = box & cv::Rect(0, 0, frame.cols, frame.rows);
        if (safe_box.area() <= 0) return full_mask;
        ...
        // Define Colors based on your 7 Classes
        // 0: Blue flare, 1: Blue pail -> BLUE
        // 2: Gate, 3: Orange flare, 4: Red flare, 5: Red pail, 6: Yellow flare -> RED/ORANGE
        ...
        if (class_id == 0 || class_id == 1) { 
            ...
            // RED/ORANGE/YELLOW Objects
            // Broad range to catch Gate, Flares, etc.
            lower = cv::Scalar(0, 50, 50);
            upper = cv::Scalar(40, 255, 255);
        }

        cv::Mat mask_roi;
        cv::inRange(hsv_roi, lower, upper, mask_roi);

        cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3,3));
        cv::morphologyEx(mask_roi, mask_roi, cv::MORPH_OPEN, kernel);

        mask_roi.copyTo(full_mask(safe_box));
        return full_mask;
    }
    ...
    float get_masked_depth(const cv::Mat& depth_img, const cv::Mat& mask, cv::Rect box) {
        ...
        for (int r = 0; r < depth_roi.rows; ++r) {
            ...
            for (int c = 0; c < depth_roi.cols; ++c) {
                if (m_ptr[c] > 0) { 
                    float d = d_ptr[c] * depth_scale_; // Scale Applied
                    if (std::isfinite(d) && d > 0.1f && d < 10.0f) {
                        valid_depths.push_back(d);
                    }
                }
            }
    ...
    void publish_masked_visual_cloud(const std::vector<YoloDet>& detections, const cv::Mat& depth_img, std_msgs::msg::Header header) {
        ...
        for (const auto& det : detections) {
            ...
            cv::Mat depth_roi = depth_img(box);
            cv::Mat mask_roi = det.mask(box); 

            for (int r = 0; r < depth_roi.rows; r += 2) { 
                const float* d_ptr = depth_roi.ptr<float>(r);
                const uint8_t* m_ptr = mask_roi.ptr<uint8_t>(r);

                for (int c = 0; c < depth_roi.cols; c += 2) {
                    if (m_ptr[c] > 0) { 
                        float Z = d_ptr[c] * depth_scale_; // Scale Applied
                        if (std::isfinite(Z) && Z > 0.1 && Z < 10.0) {
                            float u = box.x + c;
                            float v = box.y + r;
                            pcl::PointXYZRGB p;
        ...
        pcl::toROSMsg(cloud, output_msg);
        output_msg.header = header;
        output_msg.header.frame_id = "camera_link_optical"; 
        visual_cloud_pub_->publish(output_msg);
    }
    ...
    void init_trt() {
        ...
        // 2. Output 0 (Detection + Mask Coeffs)
        num_anchors_ = 8400;
        num_classes_ = 7; // CORRECT: 7 Classes
        
        // CRITICAL FIX: +32 allocates space for the segmentation masks
        // This prevents the Segmentation Fault (buffer overflow)
        int output_channels = 4 + num_classes_ + 32; 
        
        output_size_ = 1 * output_channels * num_anchors_ * sizeof(float);

        // 3. Output 1 (Unused Segmentation Proto-masks)
        // CRITICAL FIX: Allocated to satisfy API Usage Error
    ...
    cv::Mat preprocess_underwater(const cv::Mat& input) {
        ...
        float gray_mean = (b_mean + g_mean + r_mean) / 3.0f;
        if (b_mean > 0 && g_mean > 0 && r_mean > 0) {
            channels[0] = channels[0] * (gray_mean / b_mean); 
            channels[1] = channels[1] * (gray_mean / g_mean); 
            channels[2] = channels[2] * (gray_mean / r_mean); 
        }
        cv::merge(channels, output);
        cv::Mat lab_image;
        cv::cvtColor(output, lab_image, cv::COLOR_BGR2Lab);
    ...
    void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        ...
        // This loop reads 4 (box) + 7 (classes) = 11 floats per anchor
        // The remaining 32 floats (masks) in the buffer are ignored here.
        for(int i=0; i<num_anchors_; i++) {
            float max_score = 0; int cls_id = -1;
            ...
            det.class_id = cls_id;
            det.score = max_score;
            det.x = x; det.y = y; det.w = w; det.h = h;
        ...
        for (auto& det : detections) {
            ...
            // --- Hybrid Segmentation ---
            det.mask = get_color_segmentation(frame, box, det.class_id);
            ...
            if(visualize_) {
                ...
                if (safe_box.area() > 0) {
                    ...
                    cv::Mat roi_mask = det.mask(safe_box);
                    cv::Mat color_mask;
                    cv::cvtColor(roi_mask, color_mask, cv::COLOR_GRAY2BGR);
                    color_mask.setTo(cv::Scalar(0, 255, 0), roi_mask > 0);
                    cv::addWeighted(debug_img(safe_box), 1.0, color_mask, 0.5, 0.0, debug_img(safe_box));
        ...
        if(visualize_) {
            std::string fps_text = "FPS: " + std::to_string((int)current_fps_);
            cv::putText(debug_img, fps_text, cv::Point(12, 32), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 0), 4);
            cv::putText(debug_img, fps_text, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
        }
...
public:
    Yolo11TrtNode() : Node("yolo11_trt_node") {
        ...
        // 8 Classes for SAUVC
        class_names_ = {"Ball", "Blue flare", "Blue pail", "Gate", "Orange flare", "Red flare", "Red pail", "Yellow flare"};
...
private:
    ...
    // --- Helper: Verify Class ID based on Color ---
    int verify_flare_color(const cv::Mat& img, const cv::Rect& bbox, int original_class_id) {
        // 1. Safety Check
        cv::Rect safe_box = bbox & cv::Rect(0, 0, img.cols, img.rows);
        if (safe_box.area() == 0) return original_class_id;

        // 2. Extract ROI
        cv::Mat roi = img(safe_box);

        // 3. Compute Average Color (BGR format in OpenCV)
        cv::Scalar avg_color = cv::mean(roi);
        float blue = avg_color[0];
        float green = avg_color[1];
        float red = avg_color[2];

        // 4. Define Logic (Adjust thresholds experimentally)
        // Red flares usually have high Red OR high intensity if they are glowing
        // Blue flares have high Blue component
    
        bool looks_red = (red > blue * 1.2) && (red > green);
        bool looks_blue = (blue > red * 1.2);

        // 5. Override Logic
        // Indices based on your list: {"Ball", "Blue flare", "Blue pail", "Gate", "Orange flare", "Red flare", "Red pail", "Yellow flare"};
        // Blue Flare ID = 1, Red Flare ID = 5 (Assuming 0-based index from your list)
    
        int ID_BLUE_FLARE = 1;
        int ID_RED_FLARE = 5;

        if (original_class_id == ID_BLUE_FLARE || original_class_id == ID_RED_FLARE) {
            if (looks_red) return ID_RED_FLARE;
            if(looks_blue) return ID_BLUE_FLARE;
        }
    ...
    void init_trt() {
        ...
        input_size_ = 1 * 3 * input_h_ * input_w_ * sizeof(float);
        // We assume output is [1, 84, 8400]
        num_classes_ = 8; num_anchors_ = 8400; 
        output_size_ = 1 * (num_classes_ + 4) * num_anchors_ * sizeof(float);
    ...
    void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        ...
        for(auto& d : detections) {
            // Define the integer bounding box for opencv
            cv::Rect cv_bbox(d.x - d.w/2, d.y - d.h/2, d.w, d.h);
    
            // Run the color check
            d.class_id = verify_flare_color(frame, cv_bbox, d.class_id);

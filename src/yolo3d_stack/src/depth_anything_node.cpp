    ...
    : Node("depth_anything_node", options) 
{
    ...
    // FIX: Declare depth_factor here so it is ready when the callback runs
    // This allows it to load '10.0' or '4.0' from your params.yaml
...
void DepthAnythingNode::imageCb(const sensor_msgs::msg::Image::ConstSharedPtr &msg) {
    ...
    // Step 2: INVERT the depth
    // Now this is safe because we declared it in the constructor!
    float depth_factor = this->get_parameter("depth_factor").as_double();
    cv::divide(depth_factor, depth, depth);
    ...
    // 'static' keeps the variable alive between function calls (acting as memory)
    static cv::Mat accumulated_depth; 
